...
export const isNricValid = (value: string): boolean => {
  ...
  if (prefix === undefined || digits === undefined || checksum === undefined)
    return false
  ...
  const weightedSum = weights.reduce(
    (acc, weight, idx) => {
      const stringDigit = digits[idx]
      if (stringDigit) {
        const digit = parseInt(stringDigit)
        if (typeof digit === 'number' && !isNaN(digit)) {
          return acc + weight * digit
        }
        return acc // Ignore non-numeric or NaN values
      }
      return 0
    },
    startConstant, // Start constant
  )
...
export const isMFinSeriesValid = (value: string): boolean => {
  ...
  if (digits === undefined || checksum === undefined) return false
